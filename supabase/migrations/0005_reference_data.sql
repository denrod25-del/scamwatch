...
-- Versioned reference data: official verification organizations + the starter
-- threat taxonomy. Idempotent, so it is safe in every environment (local reset,
-- CI, and hosted). Promoted from seed.sql so hosted deploys carry it too. No PII.
...
-- Starter threats from the taxonomy (Vol 0 / Vol 8). Educational summaries only.
